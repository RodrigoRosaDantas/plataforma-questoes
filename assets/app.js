...
function renderMaterials(){
  ...
  const officialMarkup=officialGroups.size?[
    '<section class="card proof-source-note proof-section-wide"><div class="proof-source-eyebrow"><span class="kicker">FONTE OFICIAL · TJDFT 2022</span><span class="status-badge status-live">'+fmt(state.officialExams.length)+' cadernos</span></div><h2>Provas oficiais do último concurso</h2><p>Os PDFs abaixo são os cadernos originais hospedados pela FGV. As 179 questões dos cadernos Tipo 1 de Técnico Administrativo, Analista Administração e Analista Área Judiciária já estão revisadas e interativas; os demais cadernos permanecem como fonte oficial até revisão individual.</p><div class="release-actions"><a class="secondary" href="'+escapeHtml(state.officialExams[0].sourcePageUrl)+'" target="_blank" rel="noreferrer">Índice oficial FGV →</a><a class="secondary" href="'+escapeHtml(state.officialExams[0].officialPortalUrl)+'" target="_blank" rel="noreferrer">Portal do TJDFT →</a></div></section>',
