-- Atomic compare-and-swap for multi-device progress state.
-- A client may update only the version it actually read; stale writers receive
-- saved=false and must reload/merge before retrying.
...
begin
  ...
  if p_expected_version = 0 then
    ...
    update public.student_progress_states
  ...
  select *
    into v_row
    from public.student_progress_states
   where profile_id = p_profile_id;
